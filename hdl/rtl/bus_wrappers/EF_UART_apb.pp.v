...
module EF_UART_apb (
	...
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) PRESCALE_REG <= 0; else if(apb_we & (PADDR[15:0]==PRESCALE_REG_ADDR)) PRESCALE_REG <= PWDATA[16-1:0];
	...
	always @(posedge PCLK or negedge PRESETn) if(~PRESETn) CONTROL_REG <= 0; else if(apb_we & (PADDR[15:0]==CONTROL_REG_ADDR)) CONTROL_REG <= PWDATA[3-1:0];
