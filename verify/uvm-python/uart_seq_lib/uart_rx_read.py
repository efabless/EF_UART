...
class uart_rx_read(UVMSequence):
    ...
    async def body(self):
        ...
        for _ in range(random.randint(1, 18)):
            await uvm_do_with(self, self.req, lambda addr: addr == adress_dict["RXDATA"], lambda kind: kind == bus_bus_item.READ, lambda data: data == 0)
            # await uvm_do_with(self, self.req, lambda addr: addr == adress_dict["im"], lambda kind: kind == bus_bus_item.WRITE, lambda data: data == 0)
