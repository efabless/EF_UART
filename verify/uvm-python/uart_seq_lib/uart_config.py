...
class uart_config(bus_seq_base):
    ...
    async def body(self):
        ...
        await self.send_req(is_write=True, reg="CTRL", data_condition=lambda data: data == 0)

        # random prescale value
        if self.prescaler is not None:
            await self.send_req(is_write=True, reg="PR", data_condition=lambda data: data == self.prescaler)
        else:
            await self.send_req(is_write=True, reg="PR", data_condition=lambda data: data in range(0, 0x10))

        # random config
        if self.config is not None:
            await self.send_req(is_write=True, reg="CFG", data_condition=lambda data: data == self.config)
        else:
            await self.send_req(is_write=True, reg="CFG", data_condition=lambda data: ((data >> 8) == 0x3f and (data & 0xf) in range(5, 10) and ((data & 0xE0) >> 5) in [0, 1, 2, 4, 5] and data & 0xF == 9))
        ...
        if self.fifo_control is not None:
            await self.send_req(is_write=True, reg="FIFOCTRL", data_condition=lambda data: data == self.fifo_control)
        else:
            await self.send_req(is_write=True, reg="FIFOCTRL", data_condition=lambda data: ((data >> 8) & 0b1111) in range(0, 15) and (data & 0b1111) in range(0, 15))

        # enable uart
        if self.control is not None:
            await self.send_req(is_write=True, reg="CTRL", data_condition=lambda data: data == self.control)
        else:
            await self.send_req(is_write=True, reg="CTRL", data_condition=lambda data: data & 0b1111 == 0x7) # tx enabled, rx enabled and loopback disabled
